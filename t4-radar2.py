...
def T4Cleanup():
    try:
        print("[INFO] T4 cleaning up hung processes...")
        # Camera hanging
        subprocess.Popen('service motion stop', shell=True)
        # TO DO old deamon resetting
        
    except:
        pass

# SETUP CAMERAS
global webcam, picam, camMotion, piMotion
def T4StartCameras():
    try:
        ...
        print("[ERROR] one or more cameras failed...")




# SERIAL SETUP
global ser  # init global serial object (TO DO make array cuz might get more serial devices??)
def T4SerialSetup():
    try:
        ser = serial.Serial(
            port='/dev/ttyACM0',
        ...
        if ser.isOpen():
            ser.close()
        ser.open()
        print("T4 serial device detected")
    except:
        print("WARNING: No T4 serial device detected")
        pass
...
def T4main():
    T4StartCameras()    # Start the Cameras before starting detection
    time.sleep(2.0)
    total = 0
...
if __name__ == '__main__':
    # INIT T4
    # Cleanup old/hung deamons and services
    T4Cleanup()
    # Setup Serial
    T4SerialSetup()
